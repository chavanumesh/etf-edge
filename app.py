...
.hero-title { font-size: 24px; font-weight: 800; color: #ffffff; letter-spacing: -0.5px; margin: 0; }
.hero-sub { font-size: 13px; color: #93c5fd; margin-top: 4px; }
.hero-sync { font-size: 11px; color: #94a3b8; margin-top: 10px; display: flex; align-items: center; gap: 6px; }
.pulse {
...
.mcard { border-radius: 14px; padding: 18px 20px; position: relative; overflow: hidden; }
...
.mcard-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: rgba(255,255,255,0.65); margin-bottom: 8px; }
.mcard-value-text { font-size: 22px; font-weight: 800; color: #ffffff; letter-spacing: -0.5px; }
.mcard-delta { font-size: 12px; font-weight: 600; margin-top: 5px; display: inline-flex; align-items: center; gap: 4px; background: rgba(255,255,255,0.12); padding: 2px 8px; border-radius: 99px; color: #fff; }
.section-header { font-size: 16px; font-weight: 700; color: #f1f5f9; margin-bottom: 12px; display: flex; align-items: center; gap: 8px; }
.section-badge { font-size: 11px; background: #1d4ed8; color: #fff; padding: 2px 8px; border-radius: 99px; font-weight: 600; }
.stDataFrame { border-radius: 12px !important; overflow: hidden !important; border: 1px solid #1e293b !important; }
.stDataFrame thead tr th { background: #1e293b !important; color: #94a3b8 !important; font-size: 11px !important; font-weight: 700 !important; text-transform: uppercase !important; letter-spacing: 0.06em !important; padding: 10px 14px !important; border-bottom: 1px solid #334155 !important; }
...
.stDataFrame tbody tr td { color: #e2e8f0 !important; font-size: 13px !important; padding: 9px 14px !important; border-bottom: 1px solid #1e293b !important; }
[data-testid="stFileUploader"] { background: #1e293b !important; border-radius: 12px !important; padding: 6px !important; }
...
if df.empty:
    ...
    if "iNAV" in df.columns and "Last Traded" in df.columns:
        sig_pct    = ((df["Last Traded"] - df["iNAV"]) / df["iNAV"] * 100)
        n_discount = int((sig_pct < -0.5).sum())
        n_premium  = int((sig_pct >  0.5).sum())
    ...
    <div class="cards-row">
      ...
      <div class="mcard mcard-value">
        ...
        <div class="mcard-delta">📡 Live iNAV</div>
      </div>
      ...
      <div class="mcard mcard-holdings">
        ...
        <div class="mcard-value-text" style="font-size:18px">🟢 {n_discount} &nbsp; 🔴 {n_premium}</div>
    ...
    <div class="section-header">
      📋 Asset Matrix <span class="section-badge">{n_holdings} ETFs</span>
    </div>
    """, unsafe_allow_html=True)

    ordered_cols  = ["Name", "Quantity", "Avg Price", "Investment", "Last Traded", "iNAV", "P&L", "P&L %"]
    existing_cols = [c for c in ordered_cols if c in df.columns]
    display_df    = df[existing_cols].copy()
    display_df.rename(columns={"Investment": "Total Invested"}, inplace=True)

    # Signal column — computed on raw numeric values
    def make_signal(row):
        try:
            ltp  = float(row["Last Traded"])
            inav = float(row["iNAV"])
            if inav == 0:
                return "-"
            pct = (ltp - inav) / inav * 100
            if pct < -0.5: return f"🟢 BUY ({pct:+.2f}%)"
            if pct >  0.5: return f"🔴 SELL ({pct:+.2f}%)"
            return             f"🟡 HOLD ({pct:+.2f}%)"
        except Exception:
            return "-"

    if "iNAV" in df.columns and "Last Traded" in df.columns:
        display_df["Signal"] = df.apply(make_signal, axis=1)
    ...
    <div style="margin-top:20px; padding:14px 18px; background:#1e293b; border-radius:10px;
      ...
      <span>🟢 <b style="color:#94a3b8">BUY</b> — ETF trading below iNAV (&lt;−0.5%)</span>
      <span>🔴 <b style="color:#94a3b8">SELL</b> — ETF trading above iNAV (&gt;+0.5%)</span>
