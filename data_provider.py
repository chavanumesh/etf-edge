...
def process_data_snapshot():
    """Fetches stock data instantly and updates the cache."""
...
def update_uploaded_portfolio(df: pd.DataFrame):
    ...
    # Instant processing on upload so the UI doesn't lag out on first run
    try:
        process_data_snapshot()
    except Exception as e:
        print(f"Initial parse trigger error: {e}")
...
def init_portfolio_pipeline():
    if not any(t.name == "PortfolioFetcher" for t in threading.enumerate()):
        # FIXED: Core Target now accurately points to the proper function worker string
